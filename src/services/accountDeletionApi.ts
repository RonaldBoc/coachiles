...
export class AccountDeletionApi {
  ...
  static async getReactivationInfo(token: string): Promise<ReactivationInfo | null> {
    try {
      ...
      const now = new Date()
      const deletionDate = new Date(data.reactivation_deadline)
      const canReactivate = now < deletionDate

      if (!canReactivate) return null

      // Type assertion for coach data
      const coach = data.coaches as unknown as {
        first_name: string
        last_name: string
        user_id: string
      }
      ...
      return {
        ...
        firstName: coach.first_name,
        lastName: coach.last_name,
        deletedAt: data.deletion_requested_at,
        permanentDeletionAt: data.reactivation_deadline,
        canReactivate: true,
      }
    } catch (error) {
