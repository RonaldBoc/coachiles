...
const fetchServices = async () => {
  try {
    ...
    if (!authStore.coachId) {
      throw new Error('Coach profile required')
    }

    const { data, error: fetchError } = await supabase
      .from('services')
      .select('*')
      .eq('coach_id', authStore.coachId)
    ...
    // Map Supabase data to our Service interface
    services.value =
      data?.map((service) => ({
        id: service.id,
        name: service.name,
    ...
    error.value = err instanceof Error ? err.message : 'Erreur lors du chargement des services'
  } finally {
    loading.value = false
  }
}
    ...
    if (editingService.value) {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          duration_minutes: serviceForm.value.durationMinutes,
