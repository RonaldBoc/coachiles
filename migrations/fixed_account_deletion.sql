-- Fixed Account Deletion Migration - Corrected for actual table structure
-- Run this in your Supabase SQL Editor

-- Step 1: Add deletion tracking fields to coaches table
DO $$ 
BEGIN
...
-- For coach updates, we'll use email-based authentication since there's no user_id
CREATE POLICY "Coaches can soft delete their own profile" ON public.coaches
    FOR UPDATE TO authenticated 
    USING (email = auth.email())
    WITH CHECK (email = auth.email());

-- Step 6: Set up RLS for deletion log
...
-- Step 7: Create the deletion function (corrected for email-based authentication)
...
-- Step 8: Create the reactivation function
